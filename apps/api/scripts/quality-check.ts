...

{
  ...
  check(restGapsFor(6, 18, 20) === 3, 'при одном окне на 6 часов педагог с 18 часами получает 3 окна');
  check(restGapsFor(6, 4, 20) === 1, 'приходящий педагог с четырьмя часами получает 1 окно, а не норму большой нагрузки');
  check(restGapsFor(6, 11, 20) === 2,
    'округление к ближайшему, а не вниз: 11 часов при окне на 6 дают 2 окна, округление вниз недодало бы 1');
  check(restGapsFor(0, 18, 20) === 0, 'пока величина не названа завучем, норма не выдумывается, а обнуляется');
  check(restGapsFor(6, 18, 2) === 2, 'норма отдыха не превышает числа свободных слотов недели');
  check(

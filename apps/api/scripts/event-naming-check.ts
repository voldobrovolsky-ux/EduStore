 ...
 * 3. Wildcard-паттерны (`*`/`>`) в contract/events-файлах проверяет посегментно.
 *    Ограничение области (уточнение аудита 2026-07-28): call-sites `bus.subscribe(...)` НЕ
 *    разбираются — литерал ловится, только если лежит в contract/events-файле или создаёт
 *    событие через `newEvent()` (рантайм-fail-fast). Подписки через константы `*_EVENTS`
 *    покрыты каталогом; сырой литерал в subscribe вне contract-файла линт не увидит.
